...
///   Should be computed with k large enough to cover both near pairs and
///   mid-near candidates (recommend k >= max(n_near, mn_candidate_end)).
/// * `n_near` - Number of near pairs per point. Uses the first `n_near`
///   entries from each row of `knn_indices`. Defaults to 10 in the paper.
/// * `n_mid_near` - Number of mid-near pairs per point. Defaults to 5 in
///   the paper (MN_ratio=0.5 * n_neighbors=10).
/// * `n_further` - Number of further (random) pairs per point. Defaults to
///   20 in the paper (FP_ratio=2.0 * n_neighbors=10).
...
mod test_pacmap_pairs {
    ...
    fn test_near_pairs_count() {
        ...
        let pairs = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 42);
        assert_eq!(pairs.near.len(), 10 * 10);
    }
    ...
    fn test_mid_near_pairs_count() {
        let knn = dummy_knn(10, 50);
        let pairs = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 42);
        assert_eq!(pairs.mid_near.len(), 10 * 5);
    }
    ...
    fn test_further_pairs_count() {
        let knn = dummy_knn(10, 50);
        let pairs = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 42);
        assert_eq!(pairs.further.len(), 10 * 20);
    }
    ...
    #[test]
    fn test_near_pairs_drawn_from_first_n_near() {
        let n = 10;
        let k = 50;
        let n_near = 10;
        let knn = dummy_knn(n, k);
        let pairs = construct_pacmap_pairs(&knn, n_near, 5, 20, 4, 50, 42);
        for &(i, j) in &pairs.near {
            assert!(
                knn[i][..n_near].contains(&j),
                "near pair ({}, {}) not from first {} neighbours",
                i,
                j,
                n_near
            );
    ...
    fn test_mid_near_drawn_from_candidate_window() {
        ...
        let knn = dummy_knn(n, k);
        let pairs = construct_pacmap_pairs(&knn, 10, 5, 20, candidate_start, candidate_end, 42);
    ...
    fn test_reproducibility() {
        let knn = dummy_knn(20, 50);
        let pairs_a = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 99);
        let pairs_b = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 99);
    ...
    fn test_different_seeds_differ() {
        let knn = dummy_knn(20, 50);
        let pairs_a = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 1);
        let pairs_b = construct_pacmap_pairs(&knn, 10, 5, 20, 4, 50, 2);
    ...
    fn test_fallback_when_k_smaller_than_candidate_start() {
        let knn: Vec<Vec<usize>> = (0..10)
            ...
            .collect();
        let pairs = construct_pacmap_pairs(&knn, 3, 2, 2, 4, 50, 42);
        for &(i, j) in &pairs.mid_near {
            assert_ne!(i, j);
            assert!(j < 10);
        }
