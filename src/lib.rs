    ...
    NNDescent<T>: ApplySortedUpdates<T> + NNDescentQuery<T>,
{
    ...
    //         match init_type {
    //             #[allow(unused)]
    //             EmbdInit::PcaInit { randomised } => "pca",
    //             EmbdInit::RandomInit => "random",
    //             EmbdInit::SpectralInit => "spectral",
    //         }
    //     );
    // }
...
mod tsne_full_tests {
    ...
    fn tsne_integration_03_symmetrisation() {
        ...
        let directed =
            gaussian_knn_affinities(&knn_indices, &knn_dist, perplexity, 1e-5, 200, true);
    ...
    fn tsne_integration_05_initialisation() {
        ...
        for init_name in &["pca", "random", "spectral"] {
            ...
            let std_dev = {
                ...
                let var =
                    coords.iter().map(|&x| (x - mean).powi(2)).sum::<f64>() / coords.len() as f64;
                var.sqrt()
            };
    ...
    fn tsne_integration_07_reproducibility() {
        ...
        let embd2 = tsne(data.as_ref(), &params, 42, false);
    ...
    fn tsne_integration_08_different_seeds() {
        ...
        let embd1 = tsne(data.as_ref(), &params, 42, false);
        let embd2 = tsne(data.as_ref(), &params, 123, false);
