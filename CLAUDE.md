...
- `large_scale_diagnostics` — dev-only, gates `tests/tsne_large_scale_diag.rs` and the timing benchmarks in `tests/spectral_bench.rs`. Put heavy tests here so CI never compiles them.
