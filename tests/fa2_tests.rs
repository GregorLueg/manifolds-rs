...
// (github.com/bhargavchippada/forceatlas2, loop backend, no Barnes-Hut) on
// `parity_graph()` from `parity_pos()`. Generated with
// `uv run --with ~/repos/others/forceatlas2 --with numpy --with scipy --with tqdm python fa2_ref.py`,
// modes: linear defaults; linLogMode; strongGravityMode with gravity 0.5 and
// scalingRatio 3; edgeWeightInfluence 0.5 with jitterTolerance 0.8.
